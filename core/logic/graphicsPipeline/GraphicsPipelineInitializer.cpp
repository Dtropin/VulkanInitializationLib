...
GraphicsPipelineInterface* GraphicsPipelineInitializer::createGraphicsPipeline(VkDevice device, SwapChainHandler swapChainHandler, VkRenderPass renderPass) {
	...
	//Shader module - оболочка шейдерного кода, которую можно сунуть в GraphicsPipeline, причем после этого они уже будут не нужны
	...
	//Создаем инфо о шейдере вершин для пайплайна
    VkPipelineShaderStageCreateInfo vertShaderStageInfo = {};
    vertShaderStageInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    vertShaderStageInfo.stage = VK_SHADER_STAGE_VERTEX_BIT;
    vertShaderStageInfo.module = vertShaderModule;
    vertShaderStageInfo.pName = "main";//Имя точки входа в шейдере, причем можно задать несколько шейдеров, указав разные точки входа
	...
	//Далее тоже буфер вершин
    VkPipelineInputAssemblyStateCreateInfo inputAssembly = {};
    inputAssembly.sType = VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO;
    inputAssembly.topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST; //Собственно определяем как интерпретировать вершины 
	//(в данном случае - треугольником)
	...
	//Непонятно зачем, нужно для обрезания буфера(половину отобразить например)
    VkRect2D scissor = {};
    scissor.offset = {0, 0};
    scissor.extent = swapChainHandler.swapChainExtent;
    
	//собственно сам viewPort объект 
	VkPipelineViewportStateCreateInfo viewportState = {};
    viewportState.sType = VK_STRUCTURE_TYPE_PIPELINE_VIEWPORT_STATE_CREATE_INFO;
    viewportState.viewportCount = 1;
    viewportState.pViewports = &viewport; //Здесь можно нескоко вьюпортов задать (хз зачем)
    ...
    rasterizer.depthClampEnable = VK_FALSE;//Объекты что за Far plane и near plane - дискардим, вместо Clamp(Полезно для шадоу мап)
    rasterizer.rasterizerDiscardEnable = VK_FALSE;//Если тру, то тогда растерайзер вообще не вызывается
    ...
    rasterizer.lineWidth = 1.0f;//Толщина линий
	...
	//смешивание цветов, нужно на случай, когда пересечение между результатами фрагмент шейдера и уже пикселя существующего во
	//фреймбуфере
	...
	//Параметры пайплайна которые можно менять без пересоздания самого пайплайна
	...
	//Для uniform переменных в шейдерах
...
//Создаем обертку над шейдерным кодом
